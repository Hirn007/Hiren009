...
<div class="max-w-2xl mx-auto px-4 py-8">
    <div class="mb-6 slide-up">
        ...
        <h1 class="font-display text-3xl font-bold gradient-text">Edit Category</h1>
        <p class="text-gray-400 mt-1">Update category details</p>
    </div>

    <form action="{{ route('categories.update', $category) }}" method="POST" class="glass rounded-2xl p-6 slide-up">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-300 mb-2">Category Name <span class="text-accent-400">*</span></label>
            <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" placeholder="e.g., T-Shirts"
        ...
        <div class="flex gap-4">
            <button type="submit" class="bg-brand-600 hover:bg-brand-500 text-white px-6 py-3 rounded-xl font-medium transition-all">Update</button>
